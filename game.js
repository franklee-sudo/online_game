...
function spawnDrops(enemy) {
  const commonXp = enemy.type === "boss" ? XP_BOSS : enemy.type === "elite" ? XP_ELITE : XP_NORMAL;
  game.drops.push({
    type: "xp",
  ...
  if (Math.random() < (enemy.type === "boss" ? 1 : 0.42)) {
    game.drops.push({
      type: "gold",
      x: enemy.x + (Math.random() - 0.5) * 24,
      y: enemy.y + (Math.random() - 0.5) * 24,
      value: enemy.type === "boss" ? 30 : enemy.type === "elite" ? 5 : 2,
      radius: 8,
      attract: false,
    });
  }
  ...
  for (const enemy of game.enemies) {
    ...
    if (enemy.type === "boss") {
      ...
      if (enemy.chargeCd <= 0 && !enemy.isCharging) {
        ...
        enemy.vx = Math.cos(angle) * 360;
        enemy.vy = Math.sin(angle) * 360;
        enemy.chargeCd = 6;
      ...
      if (enemy.radialCd <= 0) {
        ...
        enemy.radialCd = 8;
        addFloatingText("弹幕！", enemy.x, enemy.y - 38, "#ff8585", 22);
      }
